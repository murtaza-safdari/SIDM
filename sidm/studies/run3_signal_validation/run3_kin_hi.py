...
ERA = os.environ.get("RUN3_ERA", "2022")   # era of the Run 3 samples
NF = 12
...
if os.path.exists(NPZ) and "--replot" in sys.argv:
    ...
    for chan, pts in POINTS.items():
        ...
        for sample, mbs in pts:
            r = V.analyze(sample, ERA, cfg, nfiles=NF)
...
for chan in ["4Mu", "2Mu2E"]:
    ...
    fig.suptitle(f"Run 3 ({ERA}) {chan} gen kinematics vs boost ($M_{{Z_d}}$=1.2 GeV, high stats)", y=0.99, fontsize=15)
    fig.tight_layout(); fig.savefig(f"{RO}/kin_hi_{chan}_{ERA}.png", dpi=110, bbox_inches="tight")
