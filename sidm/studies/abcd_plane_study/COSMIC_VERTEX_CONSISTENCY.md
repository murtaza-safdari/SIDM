...
- **cos(alpha) < -0.98** on the leading mu-LJ (partner pool = event muons not
  ...
  signal anywhere (at most ~0.21% across the ctau grid, notebook 09) while tagging the cosmic
...
- Computed on **DSA constituents**: they carry a full 3D reference point (vx,vy,vz);
  ...
  topology is DSA-dominated. This keeps the variable well defined and cheap.
...
2. **Cosmic rejection** — in the cosmic-enriched data control region (the
   cos(alpha)-tagged data control region of the continuation): does coll -> 1 concentrate
...
(signal MC + the existing cosmic CR), not a full-scale campaign, but they should
wait until the large background campaign finishes so they do not compete for slots. On
